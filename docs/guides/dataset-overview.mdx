...
- Set `mask_generation_prompt: true` when fine-tuning a hybrid-reasoning model (Nemotron, Qwen3, GLM, ...) on non-thinking data. Those chat templates insert an empty reasoning block (for example `<think></think>`) into every assistant turn that has no `reasoning_content`; at inference the generation prompt supplies that block, so training on it only teaches the model to close the block immediately. The option masks the role header and that block for every assistant turn. It is detected by rendering the template's generation prompt, so no tag strings are hardcoded. Like `answer_only_loss_mask` without `{% generation %}` blocks, it locates turns by rendering conversation prefixes, so multi-turn conversations need a template that renders earlier turns the same way as the conversation grows, or a `{% generation %}` template. Stock Qwen3 rewrites earlier assistant turns (it drops their reasoning block once a later user turn follows), so on that template multi-turn answer-only masking raises and the option applies to single-turn conversations only
